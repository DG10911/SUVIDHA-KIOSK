...
import {
  ...
  waterBills,
  staffProfiles,
  faceDescriptorSamples,
  locationStaffAssignments,
} from "../shared/schema";
...
const categoryToDepartment: Record<string, string> = {
  "power outage": "CSPDCL - Raipur Division",
  "voltage issue": "CSPDCL - Raipur Division",
  "meter problem": "CSPDCL - Raipur Division",
  "transformer": "CSPDCL - Raipur Division",
  "streetlight": "CSPDCL - Raipur Division",
  "gas leak": "Gas Authority - Chhattisgarh",
  "gas supply": "Gas Authority - Chhattisgarh",
  "pipeline": "Gas Authority - Chhattisgarh",
  "water supply": "PHE Department - Raipur",
  "water quality": "PHE Department - Raipur",
  "water leakage": "PHE Department - Raipur",
  "borewell": "PHE Department - Raipur",
  "sewage": "Municipal Corp - Sanitation Dept",
  "garbage": "Municipal Corp - Sanitation Dept",
  "drain": "Municipal Corp - Sanitation Dept",
  "cleanliness": "Municipal Corp - Sanitation Dept",
  "road damage": "Municipal Corp - Engineering Dept",
  "pothole": "Municipal Corp - Engineering Dept",
  "bridge": "Municipal Corp - Engineering Dept",
  "construction": "Municipal Corp - Engineering Dept",
  "building": "Municipal Corp - Engineering Dept",
  "encroachment": "Municipal Corp - Engineering Dept",
};
...
function normalizeDescriptor(desc: number[]): number[] {
  let mag = 0;
  for (const v of desc) mag += v * v;
  mag = Math.sqrt(mag);
  if (mag === 0) return desc;
  return desc.map(v => v / mag);
}

async function saveFaceDescriptorSamples(
  userId: string,
  descriptors: number[][],
  deviceType: string,
  lightingCondition: string = "normal"
) {
  const MAX_SAMPLES = 50;
  const DEDUPE_THRESHOLD = 0.15;

  const existingSamples = await db
    .select()
    .from(faceDescriptorSamples)
    .where(eq(faceDescriptorSamples.userId, userId));

  const existingDescs: number[][] = [];
  for (const s of existingSamples) {
    try { existingDescs.push(JSON.parse(s.descriptor)); } catch {}
  }

  const newToInsert: number[][] = [];
  for (const desc of descriptors) {
    const normalized = normalizeDescriptor(desc);
    let isDuplicate = false;
    for (const existing of existingDescs) {
      if (euclideanDist(normalized, existing) < DEDUPE_THRESHOLD) {
        isDuplicate = true;
        break;
      }
    }
    if (!isDuplicate) {
      newToInsert.push(normalized);
      existingDescs.push(normalized);
    }
  }

  const totalAfterInsert = existingSamples.length + newToInsert.length;
  if (totalAfterInsert > MAX_SAMPLES) {
    const toDelete = totalAfterInsert - MAX_SAMPLES;
    const oldest = existingSamples
      .sort((a, b) => new Date(a.capturedAt).getTime() - new Date(b.capturedAt).getTime())
      .slice(0, toDelete);
    for (const old of oldest) {
      await db.delete(faceDescriptorSamples).where(eq(faceDescriptorSamples.id, old.id));
    }
  }

  for (const normalized of newToInsert) {
    const quality = computeDescriptorQuality(normalized);
    await db.insert(faceDescriptorSamples).values({
      userId,
      descriptor: JSON.stringify(normalized),
      deviceType,
...
function computeDescriptorQuality(desc: number[]): number {
  let sum = 0;
  let sumSq = 0;
  for (const v of desc) {
    sum += v;
  ...
  const variance = sumSq / desc.length - mean * mean;
...
async function findBestFaceMatch(
...
): Promise<{ userId: string; distance: number; matchType: string } | null> {
  const normalized = normalizeDescriptor(faceDescriptor);

  const userDistances: Record<string, { distances: number[]; source: string }> = {};

  const allSamples = await db
    .select()
    .from(faceDescriptorSamples)
    .where(excludeUserId ? sql`${faceDescriptorSamples.userId} != ${excludeUserId}` : sql`1=1`);

  for (const sample of allSamples) {
    try {
      const stored: number[] = JSON.parse(sample.descriptor);
      if (stored.length !== normalized.length) continue;
      const dist = euclideanDist(normalized, stored);
      if (!userDistances[sample.userId]) userDistances[sample.userId] = { distances: [], source: "multi_sample" };
      userDistances[sample.userId].distances.push(dist);
    } catch { continue; }
  }
  ...
  const allProfiles = await db
    ...
    .where(
      excludeUserId
        ? sql`${faceProfiles.faceDescriptor} IS NOT NULL AND ${faceProfiles.userId} != ${excludeUserId}`
        : sql`${faceProfiles.faceDescriptor} IS NOT NULL`
    );

  for (const profile of allProfiles) {
    try {
      const stored: number[] = JSON.parse(profile.faceDescriptor!);
      if (stored.length !== faceDescriptor.length) continue;
      const distRaw = euclideanDist(faceDescriptor, stored);
      const distNorm = euclideanDist(normalized, normalizeDescriptor(stored));
      const dist = Math.min(distRaw, distNorm);
      if (!userDistances[profile.userId]) userDistances[profile.userId] = { distances: [], source: "legacy_profile" };
      userDistances[profile.userId].distances.push(dist);
      if (userDistances[profile.userId].source === "multi_sample") {
        userDistances[profile.userId].source = "combined";
      }
    } catch { continue; }
  }

  let bestMatch: { userId: string; distance: number; matchType: string } | null = null;

  for (const [uid, data] of Object.entries(userDistances)) {
    data.distances.sort((a, b) => a - b);
    const topK = data.distances.slice(0, Math.min(3, data.distances.length));
    const avgDist = topK.reduce((a, b) => a + b, 0) / topK.length;

    if (!bestMatch || avgDist < bestMatch.distance) {
      bestMatch = { userId: uid, distance: avgDist, matchType: data.source };
    }
  }
...
export async function registerRoutes(
  ...
  app.post("/api/auth/signup", async (req: Request, res: Response) => {
    try {
      const { aadhaar, name, phone, faceImage, faceDescriptor, address } = req.body;
      ...
      const deviceType = req.body.deviceType || (req.body.isMobile ? "mobile" : "desktop");
      const allDescriptors: number[][] = req.body.faceDescriptors || (faceDescriptor ? [faceDescriptor] : []);

      if (allDescriptors.length > 0 && allDescriptors[0].length > 0) {
        const DUPLICATE_THRESHOLD = 0.45;
        const dupCheck = await findBestFaceMatch(allDescriptors[0]);
        if (dupCheck && dupCheck.distance < DUPLICATE_THRESHOLD) {
          res.status(409).json({
            success: false,
        ...
        if (allDescriptors.length > 0 && allDescriptors[0].length > 0) {
          if (existingFace.length > 0) {
            await db.update(faceProfiles)
              .set({ faceDescriptor: JSON.stringify(allDescriptors[0]), faceHash })
            ...
            await db.insert(faceProfiles).values({
              ...
              faceImage: "stored",
              faceHash,
              faceDescriptor: JSON.stringify(allDescriptors[0]),
            });
          }
          await saveFaceDescriptorSamples(existingUser.id, allDescriptors, deviceType);
        }
      ...
      if (allDescriptors.length > 0 && allDescriptors[0].length > 0) {
        ...
        await saveFaceDescriptorSamples(user.id, allDescriptors, deviceType);
      } else if (faceImage) {
        await db.insert(faceProfiles).values({
          userId: user.id,
          faceImage: "stored",
  ...
  app.post("/api/auth/face-login", async (req: Request, res: Response) => {
    try {
      ...
      const clientMobile = req.body.isMobile === true;
      const isMobile = uaMobile || clientMobile;
      const deviceType = req.body.deviceType || (isMobile ? "mobile" : "desktop");

      const bestMatch = await findBestFaceMatch(faceDescriptor);
      ...
      const MATCH_THRESHOLD = isMobile ? 0.75 : 0.6;
      const samplesCount = await db.select({ count: count() }).from(faceDescriptorSamples).where(eq(faceDescriptorSamples.userId, bestMatch.userId));
      const hasSamples = samplesCount[0]?.count > 0;
      const effectiveThreshold = hasSamples ? MATCH_THRESHOLD + 0.05 : MATCH_THRESHOLD;

      console.log(`[FaceLogin] Match: dist=${bestMatch.distance.toFixed(3)}, type=${bestMatch.matchType}, threshold=${effectiveThreshold}, mobile=${isMobile}, samples=${samplesCount[0]?.count || 0}`);
      ...
      if (bestMatch.distance > 0.3) {
        try {
          await saveFaceDescriptorSamples(user.id, [faceDescriptor], deviceType, "login_capture");
          console.log(`[FaceLogin] Auto-learned new sample for user ${user.suvidhaId} (dist=${bestMatch.distance.toFixed(3)})`);
        } catch (e) {
          console.warn("[FaceLogin] Failed to auto-learn:", e);
        }
      }
      ...
      res.json({
        ...
        confidence: Math.round((1 - bestMatch.distance / effectiveThreshold) * 100),
        samplesStored: samplesCount[0]?.count || 0,
      });
    } catch (error: any) {
  ...
  app.post("/api/complaints", async (req: Request, res: Response) => {
    try {
      const { userId, service, category, description, urgency, latitude, longitude, locationAddress, ward, wardId, cityId, stateId } = req.body;
      ...
      const matchedDepartment = categoryToDepartment[categoryLower] || departmentMap[serviceLower] || departmentMap["other"];
      const assignedTo = matchedDepartment;
      ...
      try {
        if (wardId || cityId) {
          const assignments = await db.select({
            ...
            wardId: locationStaffAssignments.wardId,
            cityId: locationStaffAssignments.cityId,
          }).from(locationStaffAssignments)
            .where(eq(locationStaffAssignments.active, true));

          let wardStaff = wardId
            ? assignments.filter(a => a.wardId === wardId && a.roleType === "staff")
            : [];

          let deptWardStaff = wardStaff.filter(a => {
            const dept = (a.department || "").toLowerCase();
            return dept.includes(serviceLower) || dept.includes(categoryLower) ||
              Object.entries(categoryToDepartment).some(([k, v]) =>
                categoryLower.includes(k) && (a.department || "") === v
              );
          });

          if (deptWardStaff.length === 0 && wardStaff.length > 0) deptWardStaff = wardStaff;

          if (deptWardStaff.length === 0 && cityId) {
            const cityStaff = assignments.filter(a =>
              a.cityId === cityId && a.roleType === "staff" && !a.wardId
            );
            if (cityStaff.length > 0) deptWardStaff = cityStaff;
          }

          if (deptWardStaff.length > 0) {
            const chosen = deptWardStaff[Math.floor(Math.random() * deptWardStaff.length)];
            assignedStaffId = chosen.userId;
            const [staffUser] = await db.select().from(users).where(eq(users.id, chosen.userId));
            assignedStaffName = staffUser?.name || "Staff";
          }
        }

        if (!assignedStaffId) {
          const allStaff = await db.select().from(staffProfiles).where(eq(staffProfiles.active, true));
          let matchedStaff = allStaff.filter(s => s.department === matchedDepartment);
          if (ward && matchedStaff.length > 0) {
            const wardMatch = matchedStaff.filter(s => s.ward && s.ward.toLowerCase() === ward.toLowerCase());
            if (wardMatch.length > 0) matchedStaff = wardMatch;
          } else if (locationAddress && matchedStaff.length > 0) {
            const addrLower = (locationAddress || "").toLowerCase();
            const locMatch = matchedStaff.filter(s => s.ward && addrLower.includes(s.ward.toLowerCase()));
            if (locMatch.length > 0) matchedStaff = locMatch;
          }
          if (matchedStaff.length === 0 && allStaff.length > 0) {
            matchedStaff = allStaff.filter(s => s.department === matchedDepartment);
          }
          if (matchedStaff.length > 0) {
            const chosen = matchedStaff[Math.floor(Math.random() * matchedStaff.length)];
            assignedStaffId = chosen.userId;
            const [staffUser] = await db.select().from(users).where(eq(users.id, chosen.userId));
            assignedStaffName = staffUser?.name || chosen.designation;
          }
        }
      ...
      const routingNote = assignedStaffName
        ? `Complaint auto-routed to ${assignedTo} and assigned to ${assignedStaffName}. SLA deadline: ${slaDeadline.toLocaleString()}.`
        : `Complaint submitted and assigned to ${assignedTo}. SLA deadline: ${slaDeadline.toLocaleString()}.`;
      ...
      await db.insert(notifications).values({
        ...
        message: `Your complaint ${complaintId} for ${service} - ${category} has been registered and assigned to ${assignedTo}${assignedStaffName ? ` (${assignedStaffName})` : ""}.`,
        read: false,
  ...
  app.post("/api/user/:userId/register-face", async (req: Request, res: Response) => {
    try {
      ...
      const DUPLICATE_THRESHOLD = 0.45;
      const dupCheck = await findBestFaceMatch(allDescriptors[0], userId);
      if (dupCheck && dupCheck.distance < DUPLICATE_THRESHOLD) {
        res.status(409).json({
          success: false,
          message: "This face is already registered with another account. Duplicate face registrations are not allowed.",
      ...
      const faceHash = crypto.createHash("sha256").update(JSON.stringify(allDescriptors[0])).digest("hex");
      ...
      await saveFaceDescriptorSamples(userId, allDescriptors, deviceType);

      const totalSamples = await db.select({ count: count() }).from(faceDescriptorSamples).where(eq(faceDescriptorSamples.userId, userId));

      res.json({ success: true, message: "Face registered successfully", samplesStored: totalSamples[0]?.count || 0 });
    } catch (error: any) {
  ...
  // ==================== FACE LEARN (ADD MORE SAMPLES) ====================
  app.post("/api/user/:userId/face-learn", async (req: Request, res: Response) => {
    try {
      const { userId } = req.params;
      const allDescriptors: number[][] = req.body.faceDescriptors || (req.body.faceDescriptor ? [req.body.faceDescriptor] : []);
      const deviceType = req.body.deviceType || (req.body.isMobile ? "mobile" : "desktop");
      const lightingCondition = req.body.lightingCondition || "normal";

      if (allDescriptors.length === 0) {
        res.status(400).json({ success: false, message: "Face descriptors required" });
        return;
      }
      ...
      await saveFaceDescriptorSamples(userId, allDescriptors, deviceType, lightingCondition);

      const totalSamples = await db.select({ count: count() }).from(faceDescriptorSamples).where(eq(faceDescriptorSamples.userId, userId));

      res.json({
        success: true,
        message: `Added ${allDescriptors.length} face sample(s). Total: ${totalSamples[0]?.count || 0}`,
        samplesStored: totalSamples[0]?.count || 0,
      });
    } catch (error: any) {
      console.error("Face learn error:", error);
      res.status(500).json({ success: false, message: error.message || "Failed to learn face" });
    }
  });
  ...
  app.get("/api/user/:userId/face-status", async (req: Request, res: Response) => {
    try {
      ...
      const samples = await db.select({ count: count() }).from(faceDescriptorSamples).where(eq(faceDescriptorSamples.userId, userId));

      res.json({
        success: true,
        hasFace: !!profile?.faceDescriptor,
        samplesStored: samples[0]?.count || 0,
      });
    } catch (error: any) {
      res.status(500).json({ success: false, message: error.message });
    }
  ...
  // ==================== GOOGLE PLACES NEARBY SEARCH (LIVE DATA) ====================
  ...
  app.get("/api/places/nearby", async (req: Request, res: Response) => {
    try {
      ...
      const key = process.env.GOOGLE_MAPS_API_KEY;
      if (!key) return res.status(500).json({ success: false, message: "Google Maps API key not configured" });

      const searchRadius = parseInt(radius as string) || 10000;
      const placeType = (type as string) || "hospital";

      let url = `https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=${lat},${lng}&radius=${searchRadius}&type=${placeType}&key=${key}`;
      if (keyword) {
        url += `&keyword=${encodeURIComponent(keyword as string)}`;
      }
      ...
      res.json({ success: true, places, total: places.length });
    } catch (e: any) {
      console.error("[Places Nearby] Error:", e.message);
  ...
  app.get("/api/places/details", async (req: Request, res: Response) => {
    try {
      ...
      const url = `https://maps.googleapis.com/maps/api/place/details/json?place_id=${placeId}&fields=name,formatted_address,formatted_phone_number,opening_hours,geometry,rating,user_ratings_total,website,url&key=${key}`;
      const response = await fetch(url);
      const data = await response.json();
